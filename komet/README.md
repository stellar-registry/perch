...
the bundled Soroban semantics) from RV's public `k-framework` Cachix cache — so
nothing compiles from source and no separate `komet-kdist build` step is
needed. It then builds the contract with `cargo`, strips the DataCount section
...
properties, so CI runs the fuzzer and the symbolic proof is a documented local
...
1. **DataCount section.** Modern `wasm32v1-none` output carries a DataCount
   ...
   identical, and every custom section (including the `contractspecv0` ABI
   Komet reads) is preserved.
...
**A plain local Homebrew setup can't build the semantics** (the reason CI uses
Nix): Komet v0.1.88 pins **K 7.1.323** (`deps/k_release`), but Homebrew's
...
stronger symbolic check but exceeds CI's time budget — run it locally.
